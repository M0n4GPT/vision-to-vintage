...
class StyleTransferDataset(Dataset):
    def __init__(self, content_dir, style_root, transform):
        ...
        # style images per class subfolder
...
class Stylizer(nn.Module):
    def __init__(self, model, style_dir, transform, device):
        ...
        self.model = model
        self.device = device
        # load one representative style per class
        self.style_feats = []
        ...
        for label in style_dirs:
            ...
            with torch.no_grad():
                feat = self.model.enc(img)
            self.style_feats.append(feat)

    def forward(self, content, style_label):
        # content: tensor Bx3xHxW, style_label: int or tensor
        if isinstance(style_label, int):
            feat = self.style_feats[style_label]
            feat = feat.repeat(content.size(0),1,1,1)
        else:
            feat = torch.stack([self.style_feats[i] for i in style_label], dim=0)
...
def main():
    ...
    loader = DataLoader(ds, batch_size=args.micro_batch_size, sampler=sampler, shuffle=not distributed, num_workers=4, pin_memory=True)
