...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Local process rank (injected by torch.distributed launch)'
    )

    args = parser.parse_args()
    print(f"[DEBUG] local_rank={args.local_rank}, strategy={args.strategy}")
    ...
    # Tell FSDP to ignore your encoder module

    # 2) wrap for DDP if needed
    ...
    if args.strategy=='fsdp':
        ...
        # )
        # auto-wrap every Conv2d (and bias) as its own FSDP unit
        auto_wrap = partial(size_based_auto_wrap_policy, min_num_params=1)
        model = FSDP(
            model,
            auto_wrap_policy=auto_wrap,
            cpu_offload=CPUOffload(False),
            sharding_strategy=ShardingStrategy.FULL_SHARD,
            use_orig_params=True,      # keep each param’s shape
        )

    # base_model = model  # 你顶层没 wrap，就直接 model
    # opt = optim.Adam(base_model.dec.parameters(), lr=args.lr)
